...
impl Serialize for JsObject {
    fn serialize(&self, s: &mut SnapshotSerializer) -> SnapshotResult<()> {
        ...
        match s.internal_reference.entry(ptr as usize) {
            indexmap::map::Entry::Occupied(entry) => {
                let index = *entry.get();
                Reference::new(false, index).serialize(s)?;
            }
            indexmap::map::Entry::Vacant(entry) => {
                let index =
                    *entry.insert((s.bytes.len() + size_of::<u8>() + size_of::<u32>()) as u32);
                Reference::new(true, index).serialize(s)?;
